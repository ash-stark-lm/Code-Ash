...
const generateBinaryTree = (count) => {
  ...
  const horizontalSpacing = 80 // Base spacing between nodes at bottom level
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Calculate horizontal position with equal left/right spacing
...
const computePreorderSteps = (count) => {
  const steps = []
  const dfs = (node) => {
...
export default function PreorderTraversal() {
  ...
  const [steps, setSteps] = useState(computePreorderSteps(7))
  ...
  useEffect(() => {
    ...
    setSteps(computePreorderSteps(count))
    reset()
  }, [count])
  ...
  const codeSnippets = {
    cpp: `void preorder(int node, vector<vector<int>>& tree) {
  if (node == -1) return;
  visit(node);
  preorder(leftChild(node));
  preorder(rightChild(node));
}`,
    java: `void preorder(int node, TreeNode[] tree) {
  if (node == -1) return;
  visit(node);
  preorder(leftChild(node));
  preorder(rightChild(node));
}`,
    python: `def preorder(node, tree):
  if node == -1:
    return
  visit(node)
    ...
    <div className="min-h-screen bg-[#0d0d0d] text-white p-6 relative overflow-x-hidden">
      ...
      <div className="max-w-3xl mx-auto mb-10 text-center">
        <p className="text-white/80">
          In preorder traversal, we visit the root node first, then recursively
      ...
      {showCode && (
        <div className="absolute top-30 right-10 z-20 bg-[#111] p-4 rounded-xl border border-[#333] text-sm w-96 max-h-[70vh] overflow-auto">
          <h2 className="font-bold mb-2 text-lg">Preorder Code</h2>
          <select
            value={codeLang}
