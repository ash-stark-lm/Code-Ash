...
const DoublyLinkedListVisualizer = () => {
  ...
  const handleInsertHead = () => {
    ...
    const updated = [newNode, ...list.map((n, i) => ({ ...n }))]
    if (updated[1]) updated[1].prevId = newNode.id
    newNode.nextId = updated[1]?.id
    setList(updated)
  ...
  const handleInsertAtPosition = () => {
    ...
    const updated = list.map((n) => ({ ...n }))
    if (pos > 0) {
      const prev = updated[pos - 1]
      prev.nextId = newNode.id
      newNode.prevId = prev.id
    }
    if (pos < updated.length) {
      const next = updated[pos]
      next.prevId = newNode.id
      newNode.nextId = next.id
    }
  ...
  const handleDeleteHead = () => {
    ...
    const updated = list.slice(1).map((n) => ({ ...n }))
    if (updated[0]) updated[0].prevId = undefined
    setList(updated)
  ...
  const handleDeleteTail = () => {
    if (list.length === 0) return
    const updated = list.slice(0, -1).map((n) => ({ ...n }))
    if (updated.length > 0) updated[updated.length - 1].nextId = undefined
    setList(updated)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-black to-neutral-900 text-white px-4 py-10 flex flex-col items-center">
      <h1 className="text-3xl font-bold mb-2">Doubly Linked List Visualizer</h1>
      <p className="text-white/70 text-sm max-w-3xl text-center mb-6">
        A doubly linked list is a linear data structure where each node points
        to both its previous and next node, enabling bidirectional traversal.
      </p>
      ...
      <div className="w-full max-w-7xl flex flex-col lg:flex-row gap-10 items-start justify-center">
        <div className="flex items-center gap-4 overflow-x-auto px-4 py-4 border border-white/20 rounded-xl bg-white/5 backdrop-blur-lg min-h-[140px] h-60 custom-scrollbar w-full lg:w-3/5">
          {/* NULL ← before head */}
          <div className="flex flex-col items-center text-center">
            <span className="text-sm text-red-400 font-mono mb-1">null</span>
            ...
            <span className="text-xs text-red-400 font-mono mt-1">prev</span>
          </div>
          ...
          <AnimatePresence>
            {list.map((node, index) => (
              ...
              ${
                ...
                {/* Between nodes: next + prev arrows */}
                {index !== list.length - 1 && (
          ...
          {/* → NULL after tail */}
          <div className="flex flex-col items-center text-center">
            <span className="text-sm text-blue-400 font-mono mb-1">next</span>
            <svg width="30" height="24" viewBox="0 0 24 24" fill="none">
              <path
                d="M5 12h14M13 6l6 6-6 6"
                stroke="#3B82F6"
            ...
            <span className="text-xs text-blue-400 font-mono mt-1">null</span>
          </div>
        </div>
