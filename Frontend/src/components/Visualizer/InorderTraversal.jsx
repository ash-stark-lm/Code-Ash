...
const computeInorderSteps = (count) => {
  const steps = []
  const dfs = (node) => {
...
export default function InorderTraversal() {
  ...
  const [steps, setSteps] = useState(computeInorderSteps(7))
  ...
  useEffect(() => {
    ...
    setSteps(computeInorderSteps(count))
    reset()
  }, [count])
  ...
  const codeSnippets = {
    cpp: `void inorder(int node, vector<vector<int>>& tree) {
  if (node == -1) return;
  inorder(leftChild(node));
  visit(node);
  inorder(rightChild(node));
}`,
    java: `void inorder(int node, TreeNode[] tree) {
  if (node == -1) return;
  inorder(leftChild(node));
  visit(node);
  inorder(rightChild(node));
}`,
    python: `def inorder(node, tree):
  if node == -1:
    return
    ...
    <div className="min-h-screen bg-[#0d0d0d] text-white p-6 relative overflow-x-hidden">
      ...
      <div className="max-w-3xl mx-auto mb-10 text-center">
        <p className="text-white/80">
          In inorder traversal, we recursively visit the left subtree, then the
          root node, then the right subtree.
        </p>
      </div>
      ...
      {showCode && (
        <div className="absolute top-32 right-10 z-20 bg-[#111] p-4 rounded-xl border border-[#333] text-sm w-96 max-h-[70vh] overflow-auto">
          <h2 className="font-bold mb-2 text-lg">Inorder Code</h2>
          <select
            value={codeLang}
